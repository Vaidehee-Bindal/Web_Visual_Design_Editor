...
const newLocalId = () =>
  `local-canvas-${Date.now()}-${Math.random().toString(36).slice(2)}`;
const newElementId = () =>
...
const blankCanvas = (): CanvasDocument => ({
  localId: newLocalId(),
...
const nextDefaultName = (documents: CanvasDocument[]) => {
  const names = new Set(documents.map((document) => keyForName(document.name)));
  let index = 1;
...
export default function Editor({ canvasId }: { canvasId?: string } = {}) {
  ...
  const [activeCanvasId, setActiveCanvasId] = useState(initial.localId!);
  ...
  const activeCanvas =
    canvases.find((canvas) => canvas.localId === activeCanvasId) ||
    canvases[0] ||
    null;
  const activeCanvasRef = useRef<CanvasDocument | null>(activeCanvas);
  useEffect(() => {
    canvasesRef.current = canvases;
    activeCanvasRef.current =
      canvases.find((canvas) => canvas.localId === activeCanvasId) || null;
  ...
  const replaceCanvas = useCallback((next: CanvasDocument) => {
    ...
    activeCanvasRef.current =
      next.localId === activeCanvasIdRef.current
        ? next
        : activeCanvasRef.current;
  ...
  const updateActiveCanvas = useCallback(
    (
      ...
      const current = activeCanvasRef.current;
      if (!current) return;
      if (addHistory)
        setHistory((items) => [...items.slice(-29), current.elements]);
      setFuture([]);
  ...
  const loadCanvases = useCallback(async () => {
    ...
    try {
      ...
      const documents = saved.map((canvas) => ({
        ...canvas,
        localId: canvas._id || newLocalId(),
      }));
      let next = documents;
      let active = documents[0];
      if (canvasId) {
        const loaded = { ...(await api.get(canvasId)), localId: canvasId };
        next = [
          loaded,
          ...documents.filter((canvas) => canvas.localId !== canvasId),
        ];
        active = loaded;
      } else {
        const current = activeCanvasRef.current;
        const keepBlankActive =
          !!current && !current._id && current.elements.length > 0;
        next = keepBlankActive
          ? [current!, ...documents]
          : documents.length
            ? documents
            : [blankCanvas()];
        active = keepBlankActive ? current! : next[0];
      }
      canvasesRef.current = next;
      activeCanvasRef.current = active;
      activeCanvasIdRef.current = active.localId!;
      setCanvases(next);
      setActiveCanvasId(active.localId!);
      setSelectedIds([]);
      setStatus(
        !canvasId && !!active && !active._id && active.elements.length
          ? "Unsaved changes"
          : "All changes saved",
      );
    } catch (error) {
  ...
  const selectCanvas = useCallback(
    (localId: string) => {
      if (localId === activeCanvasIdRef.current) return;
      const target = canvasesRef.current.find(
        (canvas) => canvas.localId === localId,
      );
      if (!target) return;
      if (status === "Unsaved changes" && !autosave) {
        setPendingCanvasId(localId);
        return;
      }
      activeCanvasIdRef.current = localId;
      activeCanvasRef.current = target;
      setActiveCanvasId(localId);
      setSelectedIds([]);
      setHistory([]);
      setFuture([]);
      setEditingId(null);
  ...
  const confirmSwitch = useCallback(
    (saveFirst: boolean) => {
      const targetId = pendingCanvasId;
      setPendingCanvasId(null);
      if (!targetId) return;
      if (saveFirst) {
        void saveCanvas().then((success) => {
          if (success) selectCanvas(targetId);
        });
        return;
      }
      const target = canvasesRef.current.find(
        (canvas) => canvas.localId === targetId,
      );
      if (!target) return;
      activeCanvasIdRef.current = targetId;
      activeCanvasRef.current = target;
  ...
  const updateElements = useCallback(
    (elements: CanvasElement[]) =>
      updateActiveCanvas((canvas) => ({ ...canvas, elements })),
    [updateActiveCanvas],
  );
  ...
  const selected = useMemo(
    () =>
      activeCanvas?.elements.find((element) => element.id === selectedIds[0]),
    [activeCanvas, selectedIds],
  );
  useEffect(() => {
    if (
      editingId &&
  ...
  }, [editingId, editingText, selected]);
  ...
  const selectMany = (ids: string[], additive = false) =>
    setSelectedIds((current) =>
      additive ? Array.from(new Set([...current, ...ids])) : ids,
    );
  const addElement = (type: ElementType, lineStyle: LineStyle = "straight") => {
    const base = {
      id: newElementId(),
      x: 160,
      y: 130,
    ...
    const element: CanvasElement =
      type === "rectangle"
        ? { ...base, type, fill: "#8b5cf6", width: 190, height: 120 }
        : type === "circle"
          ? { ...base, type, radius: 68 }
          : type === "line"
            ? {
    ...
    updateElements([...(activeCanvasRef.current?.elements || []), element]);
  ...
  const newCanvas = async () => {
    try {
      ...
      const next = { ...created, localId: created._id || newLocalId() };
      const refreshed = (await api.list()).map((canvas) => ({
        ...canvas,
        localId: canvas._id || newLocalId(),
      }));
      const documents = refreshed.some(
        (canvas) => canvas.localId === next.localId,
      )
        ? refreshed
        : [...refreshed, next];
      canvasesRef.current = documents;
      ...
      setStatus(
        error instanceof Error ? error.message : "Unable to create canvas",
      );
    }
  };
    ...
    const run = async (): Promise<boolean> => {
      ...
      try {
        while (saveRequestedRef.current) {
          ...
          if (!source) continue;
          const requestedName = source.name.trim();
          const documents = canvasesRef.current;
          const duplicate = documents.some(
            (canvas) =>
              canvas.localId !== source.localId &&
              keyForName(canvas.name) === keyForName(requestedName),
          );
          const isDefault =
            !requestedName || defaultNamePattern.test(requestedName);
          if (duplicate && !isDefault) {
            successful = false;
            setStatus(`A canvas named “${requestedName}” already exists`);
            continue;
          }
          const name =
            duplicate || !requestedName
              ? nextDefaultName(
                  documents.filter(
                    (canvas) => canvas.localId !== source.localId,
                  ),
                )
              : requestedName;
          const requestSource = { ...source, name };
          const saved = requestSource._id
            ? await api.update(requestSource._id, requestSource)
            : await api.create(requestSource);
          const latest =
            activeCanvasRef.current &&
            activeCanvasRef.current.localId === source.localId
              ? activeCanvasRef.current
              : source;
          const merged = {
            ...latest,
            ...
            name: latest.name || saved.name,
          };
          canvasesRef.current = canvasesRef.current.map((canvas) =>
            canvas.localId === source.localId ? merged : canvas,
          );
  ...
  }, [autosave, status, activeCanvasId, saveCanvas]);
  const undo = () => {
    const previous = history.at(-1);
    const current = activeCanvasRef.current;
    if (!previous || !current) return;
    setFuture((items) => [...items, current.elements]);
    setHistory((items) => items.slice(0, -1));
    replaceCanvas({ ...current, elements: previous });
    markUnsaved();
  };
  const redo = () => {
    ...
    setHistory((items) => [...items, current.elements]);
    setFuture((items) => items.slice(0, -1));
    replaceCanvas({ ...current, elements: next });
    markUnsaved();
  };
  const removeSelected = () => {
    if (selectedIds.length) {
      updateElements(
        (activeCanvasRef.current?.elements || []).filter(
  ...
  const reorder = (direction: number) => {
    ...
    if (
      ordered.some(
        (index) =>
          index + direction < 0 || index + direction >= elements.length,
      )
    )
      return;
  ...
  const beginTextEdit = (id: string) => {
    const element = activeCanvasRef.current?.elements.find(
      (item) => item.id === id,
    );
  ...
  const pasteCopied = () => {
    ...
    const pasted = clipboard.map((element) => ({
      ...element,
      id: newElementId(),
      x: element.x + 24,
      y: element.y + 24,
    ...
    updateElements([...(activeCanvasRef.current?.elements || []), ...pasted]);
  ...
  const confirmRemoveCanvas = async () => {
    ...
    const remaining = canvasesRef.current.filter(
      (canvas) => canvas.localId !== localId,
    );
    if (!remaining.length) {
      ...
      canvasesRef.current = [replacement];
      activeCanvasRef.current = replacement;
      activeCanvasIdRef.current = replacement.localId!;
      setCanvases([replacement]);
      setActiveCanvasId(replacement.localId!);
      setSelectedIds([]);
      setHistory([]);
      setFuture([]);
      setEditingId(null);
    ...
    canvasesRef.current = remaining;
    setCanvases(remaining);
    if (localId === activeCanvasIdRef.current) {
  ...
  const activeIndex = activeCanvas
    ? canvases.findIndex((canvas) => canvas.localId === activeCanvas.localId)
    : -1;
  return (
    <main className="app-shell">
      <Toolbar
        ...
        onExport={() => {
          ...
          link.download = `${activeCanvas.name}.png`;
          link.href = uri;
          link.click();
        }}
        onUndo={undo}
      ...
      <div className="workspace">
        <aside className="left-rail">
          ...
          <section className="sidebar-section add-section">
            ...
            <div className="element-grid">
              {(["rectangle", "circle", "text"] as ElementType[]).map(
                (type) => (
                  <button key={type} onClick={() => addElement(type)}>
                    <span
                      className={
                        type === "circle"
                          ? "shape-icon circle-icon"
                          : type === "rectangle"
                            ? "shape-icon rectangle-icon"
                            : "text-icon"
                      }
                    >
                ...
                <div className="stage-board">
                  ...
                  {editingId && selected && (
                    <textarea
                      ...
                      value={editingText}
                      style={{
                        left: (selected.x || 0) * zoom,
                        top: (selected.y || 0) * zoom,
                        width: (selected.width || 250) * zoom,
                        height:
                          (selected.height ||
                            Math.max(selected.fontSize || 32, 32)) * zoom,
                        fontFamily: selected.fontFamily || "Arial",
                        fontSize: (selected.fontSize || 32) * zoom,
                        fontWeight: (selected.fontStyle || "").includes("bold")
                          ? 700
                          : 400,
                        ...
                        transform:
                          "rotate(" + (selected.rotation || 0) + "deg)",
        ...
        {activeCanvas && (
          <PropertiesPanel
            ...
            onChange={(patch) =>
              selectedIds[0] && updateElement(selectedIds[0], patch)
            }
      ...
      {pendingCanvasId && (
        <div className="auth-backdrop">
          <div className="auth-card">
            ...
            <h2>Save before switching?</h2>
            <p>Your current canvas has changes that have not been saved.</p>
            <button className="save-button" onClick={() => confirmSwitch(true)}>
              Save
            </button>
            <button
            ...
            <button
              className="subtle-button"
              onClick={() => setPendingCanvasId(null)}
            >
              Cancel
            </button>
          </div>
