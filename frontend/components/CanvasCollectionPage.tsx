...
export default function CanvasCollectionPage({
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(
        reason instanceof Error ? reason.message : "Unable to load canvases",
      );
    } finally {
      setLoading(false);
    }
  ...
  const restore = async (id: string) => {
    try {
      ...
      setError(
        reason instanceof Error ? reason.message : "Unable to restore canvas",
      );
    }
  };
  const remove = async (id: string) => {
    try {
      if (view === "trash") await api.permanentlyRemove(id);
      else await api.remove(id);
      await load();
    } catch (reason) {
      setError(
        reason instanceof Error ? reason.message : "Unable to delete canvas",
      );
    }
  };
  return (
    <main className="app-shell">
      ...
      <div className="collection-page">
        <div className="collection-heading">
          ...
          <h1>{title}</h1>
          <button className="save-button" onClick={() => router.push("/")}>
            Open editor
          </button>
        </div>
        ...
        <div className="collection-grid">
          {items.map((canvas) => {
            const id = canvas._id || canvas.localId!;
            return (
              <article className="collection-card" key={id}>
                <button
                  className="collection-preview"
                  onClick={() =>
                    view === "trash" ? undefined : router.push("/canvas/" + id)
                  }
                >
                  <CanvasThumbnail
