...
const elementSchema = new mongoose.Schema({
  id: { type: String, required: true }, lineStyle: { type: String, enum: ['straight', 'curved', 'elbow'] },
  ...
  fill: { type: String, required: true }, stroke: String, strokeWidth: Number, strokeEnabled: Boolean, opacity: Number, points: [Number],
  text: String, fontSize: Number, fontFamily: String, fontStyle: String, textDecoration: String, align: String,
}, { _id: false, strict: true });

const canvasSchema = new mongoose.Schema({
  name: { type: String, required: true, trim: true, maxlength: 80 },
  nameKey: { type: String, required: true, unique: true, sparse: true, select: false },
